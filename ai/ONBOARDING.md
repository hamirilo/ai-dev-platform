...
5. `application-ui-kit` を利用しているApplicationでDjango TemplateへIsland / htmxを接続・追加する場合は、Playbookの [DjangoとReact Islandsの接続](../playbook/playbooks/django-react-islands.md) に従う。見せ方だけの切替（tab、開閉、出し分け）とtemplate用の共通classのうちUI Platformが提供するComponent / Islandsで解決できるものは、Application側で重複実装しない。利用していないApplicationには3.が優先し、この項目を理由にUI Kitを追加しない。
